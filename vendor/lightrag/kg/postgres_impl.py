...
class PostgreSQLDB:
    ...
    async def ensure_age_label(
        ...
        *,
        ...
        """Create an AGE label only when it is missing for the graph.

        AGE's create_vlabel/create_elabel helpers raise duplicate-object errors
        for existing labels. Checking ag_label first avoids noisy server-side
        PostgreSQL ERROR logs during normal LightRAG container restarts.
        """
        label_exists = await connection.fetchval(  # type: ignore
            """
            SELECT EXISTS(
                SELECT 1
                FROM ag_catalog.ag_label label
                JOIN ag_catalog.ag_graph graph ON label.graph = graph.graphid
                WHERE graph.name = $1 AND label.name = $2
            )
            """,
            graph_name,
            label_name,
        ...
        if label_exists:
            return
        function_name = "create_elabel" if edge else "create_vlabel"
        await connection.execute(  # type: ignore
    ...
    async def check_tables(self):
        ...
        for k, v in TABLES.items():
            ...
            try:
                table_exists = await self.check_table_exists(k)
                if table_exists:
                    continue
...
class PGGraphStorage(BaseGraphStorage):
    ...
    async def initialize(self):
        async with get_data_init_lock():
            ...
            async def _do_configure_age(
                connection: asyncpg.Connection,
            ) -> None:
                await PostgreSQLDB.configure_age_extension(connection)
                await PostgreSQLDB.configure_age(connection, self.graph_name)

            await self.db._run_with_retry(_do_configure_age)

            async def _do_configure_age_labels(
                connection: asyncpg.Connection,
            ) -> None:
                await PostgreSQLDB.ensure_age_label(
                    connection,
                    self.graph_name,
                    "base",
                )
            ...
            await self.db._run_with_retry(_do_configure_age_labels)
