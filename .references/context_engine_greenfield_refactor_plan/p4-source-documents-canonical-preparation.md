...
Do not store:
  ...
  raw parser config
  parser profile ID
  parser config revision
```

Credential rotation may change secret value.

Parser kind remains frozen for source retry. Current credentials for that parser kind are resolved privately at retry time.
...
4. Resolve active parser kind server-side and store it on the source document.
