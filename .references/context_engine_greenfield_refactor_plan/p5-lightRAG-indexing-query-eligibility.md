...
immediate domain embedding immutability from P3
...
## 9.1 Domain embedding immutability

Use the Phase 3 domain `embedding_profile_id`. It is selected at domain creation and immutable immediately.

Do not add:

```text
embedding_locked_at
pre-index embedding edits
embedding revision logic
embedding migration workflow
```

Wrong embedding profile means hard-delete the empty domain and recreate it. P5 only confirms that the prepared source was submitted with the domain's immutable embedding profile.
...
Ready source settlement:

```text
1. Lock domain row.

2. Confirm source submit config matches the domain's immutable embedding_profile_id.

3. Complete guarded source ready update.
```
...
Prove API-to-controller provider-secret delivery before indexing.
...
Source ready settlement
  -> confirms immutable domain embedding profile.

Domain embedding profile update route
  -> does not exist in P5.
