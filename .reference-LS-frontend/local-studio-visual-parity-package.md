# Local Studio Visual-Parity Package

# Prompt — Senior UI/UX Designer: Local Studio Visual-Parity Extraction

## Role

Act as a senior product UI/UX designer, design-systems architect, and frontend design engineer.

Your task is **not** to redesign Local Studio. Your task is to reverse-engineer its live frontend visual system and produce a Context Engine design specification that achieves **visual and interaction parity** while Context Engine changes the underlying RAG/domain business logic.
...
Use the provided Context Engine scaffold and the current Context Engine `design.md` as target-product context.

## Objective

Create a complete, implementation-ready visual-parity system so coding agents can:

1. preserve Local Studio’s current visual language,
2. build Context Engine-specific screens using that language,
3. extend the UI without style drift,
4. use tokens and shared primitives instead of page-specific CSS,
5. maintain dark and light theme parity.

**Context Engine may change information architecture, labels, data, and workflows. It must not become a visual redesign.**

## Non-Negotiable Constraints

- Treat rendered Local Studio UI and its source tokens as the reference product.
- Treat existing Local Studio global CSS tokens, typography, shared primitives, and shell geometry as stronger evidence than generic UX taste.
- Preserve the current Local Studio default: a dark-first, compact, desktop workbench. Light mode is a first-class equivalent, not a separate aesthetic.
- Do not replace the system with a white-canvas, shadcn-default, SaaS-dashboard, or marketing-page aesthetic.
- Do not introduce new colors, font families, corner radii, shadows, spacing scales, component shapes, or animation patterns unless a reference implementation needs a missing state.
- Use the narrowest compatible addition when Context Engine introduces a new concept such as domains, documents, ingestion jobs, retrieval evidence, or provider settings.
- Do not use arbitrary utility values or hard-coded colors in component screens when an existing token should be used.
- Do not propose a broad frontend rewrite.
- Keep accessibility, keyboard focus, contrast, responsive behavior, and empty/loading/error states intact.

## Required Evidence Collection

Inspect and cite exact file paths and relevant code for:

1. Global styles and all theme tokens.
2. Font loading, font-family options, typography scale, line-height, weights, letter-spacing, and user-controlled scale/radius settings.
3. Shared primitives, including button, input, select, tabs, segmented control, status, table, list, cards, modal, drawer, right-detail panel, progress, and page shells.
4. App shell geometry: sidebar, collapsed sidebar, header, content canvas, detail panel, composer/thread regions.
5. At least these representative routes/surfaces:
   - agent workspace,
   - settings,
   - usage or logs,
   - model/recipe/server surfaces,
   - modal/drawer/detail-panel interactions.
6. Dark and light mode at desktop widths. Use rendered screenshots where the app can run; source-token measurements remain canonical when visual inspection conflicts with assumptions.

Prioritize these known reference areas:
...
Create a table that captures the exact system:

| Category | Local Studio evidence | Canonical decision for Context Engine | Token / primitive to reuse | Do not do |
|---|---|---|---|---|

Cover:

- dark and light surfaces,
- borders, hover, selected, focus, disabled,
- semantic state colors,
- typography roles and exact size variables,
- spacing and geometry,
- sidebar/content/detail layouts,
- radii and control heights,
- buttons, inputs, selects, tabs, segmented controls,
- cards, lists, tables, chips/statuses,
- code/metadata/paths,
- shadows, elevation, and motion,
- icons,
- empty/loading/error states,
- responsive behavior.

### 2. Parity vs. Product-Specific Change

For every Context Engine screen or component pattern, classify:

- **Direct reuse** — Local Studio component/layout can be retained with copy/data changes only.
- **Controlled extension** — Context Engine needs a new state or variant; specify exactly how it inherits existing tokens and behavior.
- **Remove / do not emulate** — Local Studio feature is domain-specific and does not belong in Context Engine.
- **Needs evidence** — reference behavior is unclear; identify the exact verification needed.

Do not label something as a “new design direction” merely because Context Engine has different business objects.

### 3. Context Engine Pattern Mapping

Define exact visual mappings for:

- Domains/workspaces
- Domain lifecycle: created, starting, ready, stopped, failed, deleting
- Document upload and parser selection
- Ingestion jobs and pipeline progress
- Document status, errors, delete actions
- Retrieval/query/chat
- Citations, chunks, sources, images/tables
- Provider and model settings
- Admin operations and logs
- Empty, loading, permission-denied, and failure states

Every mapping must use Local Studio component grammar first: sidebar/list row, page header, table, status dot/pill, detail panel, modal, drawer, compact form row, or existing tab/control pattern.

### 4. Design-System Output

Replace the supplied Context Engine `design.md` with an implementation-ready document that:

- is explicitly titled as Local Studio visual parity,
- declares Local Studio token compatibility as the governing rule,
- records exact dark/light token values and aliases,
- records font, typography, spacing, radius, geometry, and component rules,
- shows practical token-first Tailwind/class usage,
- defines reusable Context Engine patterns without introducing a rival visual language,
- includes do/don’t rules,
- includes an extension procedure for agents,
- includes a visual acceptance checklist,
- lists reference source paths.

## Required Deliverables

1. **Executive summary** — whether the supplied `design.md` aligns with Local Studio, and the highest-impact corrections.
2. **Visual evidence report** — file/path evidence, observed values, and unresolved uncertainty.
3. **Visual parity matrix** — Local Studio reference → Context Engine rule.
4. **Rewritten `design.md`** — complete replacement, ready to commit.
5. **Component extension map** — which primitives need reuse, new variants, or no change.
6. **Screen mapping** — Context Engine surfaces mapped to reference patterns.
7. **Screenshot-validation plan** — target viewport sizes, representative states, and measurable acceptance checks.
8. **Implementation sequence** — token foundation → primitives → shell → feature screens → regression checks.

## Acceptance Criteria

The work is accepted only when:

- Default Context Engine dark mode reads as Local Studio’s compact Codex-like workbench, not a white admin dashboard.
- Light mode uses the matching Local Studio light token set.
- Geist and Geist Mono, the compact type ramp, 400/500/600 weight hierarchy, and existing line-height rules are specified exactly.
- Existing Local Studio aliases (`--ui-*`, legacy aliases, and library compatibility aliases) are preserved or mapped without duplicate token systems.
- Core inline controls and dense rows target Local Studio’s compact geometry.
- New Context Engine flows use shared primitives and tokenized variants; they do not invent independent cards, gradients, color panels, full pills, or custom shadow systems.
- Each recommendation names a concrete source file or a source-token rule.
- Visual validation includes screenshots at 1440×900 and 1280×800 for dark and light mode, plus a narrow-width verification for any changed responsive shell.
- No generic redesign recommendation is included unless a reference component cannot support a required accessible state.
...
| Local Studio source reference | `frontend/src/app/styles/globals/tokens.css` |
| Local Studio shared primitives | `frontend/src/ui/` |

When instructions conflict, use this order:

1. existing Local Studio visual tokens and primitives;
2. this document;
3. feature-specific UI requirements;
4. generic shadcn/Tailwind defaults.
...
The source implementation to preserve is:
