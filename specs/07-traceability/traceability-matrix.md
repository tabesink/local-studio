...
| BR-002 cookie-only session | F-001, F-009 | API-001, QA-002 | auth integration + browser storage test | foundation implemented via cookie-native auth wrappers/store and no token storage scan; live backend auth integration pending |
...
| BR-011 visual parity | F-009 | DESIGN.md | screenshots and visual review | foundation shell/token baseline implemented; screenshot matrix and visual review pending |
...
| NFR-008 minimal runnable stack | F-010 | RUN-001, API-001, DATA-001, QA-002, QA-003 | current-repo compose/deployment smoke proving stock Postgres, migrations, API health/auth, production frontend proxy, and safe HTTP smoke evidence | implemented for first gate via `compose.p10.yml`, P10 stack smoke, and safety scan; richer node/logs/usage/storage UI remains contract-blocked |
