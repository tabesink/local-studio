...
| API/service | Implement only endpoints and services named by this feature. Conversation detail, title updates, pre-stream JSON errors, idempotent replay, and SSE payloads follow API-001/EVT-001. |
| Data | `conversations`, `conversation_turns`, `conversation_turn_evidence_refs` with evidence order/source label/private ids for redaction; no prompt/raw evidence/raw provider columns. |
...
- [x] T-030 [backend/service] Implement turn idempotency and one-running-turn guard.
  - Verification: 409/duplicate tests; completed/failed replay tests prove no second provider/retrieval call and request-conflict tests reject changed message/effective domain.
- [x] T-040 [backend/ai] Implement server intent gate and direct LLM responder for non-domain general chat.
  - Verification: direct route tests prove no retrieval/evidence/citations and domain questions fail closed without a domain.
- [x] T-050 [backend/ai] Implement CE-native advanced agentic RAG `TurnOrchestrator` with a statically registered typed middleware chain and closed RetrievalPort operations.
  - Verification: budget/operation/middleware tests; `fact`/`overview`/`verbatim` hit the same RetrievalPort/P6 path through the internal mapped-evidence bridge; invalid operation fails closed; no direct LightRAG import and no LangChain/LangGraph dependency in chat runtime.
- [x] T-060 [backend/api] Implement Context Engine SSE endpoint with safe `stage`, `evidence`, `token`, `done`, and `error` projection.
  - Verification: SSE fixture tests cover payload shape, terminal outcomes, no token events for `no_grounded_context`/`evidence_only`, and JSON errors before SSE opens.
...
- [x] T-080 [backend/eval] Add direct, no-evidence, single-hop, provider-failure, and invalid-operation fixture coverage.
  - Verification: pytest fixture cases record route, stop reason, evidence ids, and budget counters; multi-hop and budget-exhaustion runtime behavior remain deferred by LD-008 single-hop v1.
