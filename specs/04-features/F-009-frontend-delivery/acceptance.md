...
| AC-006 | `frontend/src/lib/api/errors.ts`; `frontend/src/lib/api/client.ts`; `frontend/tests/foundation.test.mjs`; `tests/test_sources.py` member preview safety | pass | client error surface is limited to safe fields and API client blocks non-CE URLs; preview list/error envelopes pass forbidden-field scans |
| AC-007 | Playwright `tests/e2e/pilot-happy-path.spec.ts` + `tests/e2e/documents-preview.spec.ts` against `compose.stack.yml` | partial | **pilot + documents preview pass** 2026-07-10 (`PLAYWRIGHT_BASE_URL=http://127.0.0.1:3010 npx playwright test tests/e2e/documents-preview.spec.ts` — 3 passed: admin markdown preview, admin PDF object, member read-only Library). Graph browser flow remains planned until graph DTOs exist |
