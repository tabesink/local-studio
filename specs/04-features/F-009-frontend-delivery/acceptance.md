...
| AC-007 | Playwright `tests/e2e/pilot-happy-path.spec.ts` + `tests/e2e/documents-preview.spec.ts` + `tests/e2e/source-ref-inspector.spec.ts` against `compose.stack.yml` | partial | **pilot + documents preview + source-ref inspector** 2026-07-10/11. Source-ref AE1/AE2 (Evidence → Library → Back to turn) and AE4 (unavailable stays in chat) covered by `source-ref-inspector.spec.ts`; AE7 (ordinary Library pdf.js) covered by `documents-preview.spec.ts` admin PDF test — not duplicated. **Green evidence 2026-07-11:** `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 COMPOSE_PROJECT_NAME=context_engine_stack npx playwright test tests/e2e/source-ref-inspector.spec.ts tests/e2e/documents-preview.spec.ts` — **6 passed** (re-run after review fixes for AE3 browser Back). When resolve returns `page`, e2e asserts URL `page` + pdf.js `data-page`. Graph browser flow remains planned until graph DTOs exist. |
