...
| 2026-07-08 | Review fixes CHG-035: delete-worker claim query uses `with_for_update(skip_locked=True)`; domain-gone exit finalizes the operation as `succeeded` and the generation-fence exit as `cancelled`, with tests for both paths. | ce-code-review #2/#3/#11 — plain SELECT-then-UPDATE claims can double-claim on multi-worker Postgres, and both early exits left operations stuck RUNNING. | None; see `docs/residual-review-findings/a85eb030.md`. |
