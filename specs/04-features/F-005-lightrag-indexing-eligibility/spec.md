...
- Native private LightRAG runtime integration must use an editable vendored copy at `vendor/lightrag/` per ADR-002. Pip-only `lightrag-hku` is not acceptable as the runtime source of truth when Context Engine-owned KG prompt changes are required. `.references/code/lightrag/` remains read-only evidence until promotion.
...
Resolved T-060: pinned reference LightRAG `1.4.16` has been promoted into `vendor/lightrag/`; the pinned native proof imports from the vendored tree; focused runtime dependencies and package discovery are declared in `pyproject.toml`.

Fixture-captured technical choices still pending after T-060 vendored runtime promotion:

- exact private delete/readiness primitive for production native runtime (P5 fake client already proves state-machine fences);
- whether `index_remote_document_id` is necessary;
- whether marker v1 needs page/section metadata.

Resolved: pinned reference version for proof fixtures is LightRAG `1.4.16`; `.references/code/lightrag/_version.py` remains read-only evidence, and `vendor/lightrag/_version.py` is the promoted runtime source.
